...
chrome_options.add_argument("--no-sandbox") # REQUIRED for Docker (security bypass)
chrome_options.add_argument("--disable-dev-shm-usage") # REQUIRED for Docker (memory fix)

driver = webdriver.Chrome(options=chrome_options)

# Setup Logging in GCP (Crucial for debugging in the cloud)
...
if not WEATHERBIT_KEY:
    logger.warning("No API Key found! Check your .env file.")

# Define Data Models (Pydantic)
# This ensures the data coming from the mobile app is valid before you process it
class Coordinates(BaseModel):
    latitude: float
    longitude: float

# FastAPI Lifespan (modern way to handle startup/shutdown)
@asynccontextmanager
async def lifespan(app: FastAPI):
    logger.info("ThorsHammer Backend Starting...")
    # Any database connections or scheduled tasks start here
    yield
    logger.info("ThorsHammer Backend Shutting Down...")

app = FastAPI(lifespan=lifespan)

# Core Endpoint
@app.post("/check-risk")
async def check_weather_risk(coords: Coordinates):
    """
    Mobile App sends GPS coords -> Python fetches specific data -> Returns simplified risk assessment.
    """
    try:
        # Fetch Alerts
        alerts_url = f"{BASE_URL}/alerts?lat={coords.latitude}&lon={coords.longitude}&key={WEATHERBIT_KEY}"
        # Use a timeout so your script doesn't hang forever if Weatherbit is down
        response = requests.get(alerts_url, timeout=10) 
        response.raise_for_status()
        data = response.json()
        
        # Simple Logic to determine "Risk"
        # You can expand this with Lightning data later
        active_alerts = data.get('alerts', [])
        risk_level = "HIGH" if active_alerts else "LOW"
...
# Run logic (only for local testing)
if __name__ == "__main__":
    import uvicorn
    # In production, you don't run python main.py. 
...
def derive_condition(weather_data):
    ...
    if any(term in description for term in ['thunderstorm', 'tornado', 'hurricane', 'storm', 'extreme']):
        return "Severe Weather"
    elif 'lightning' in description or 'electrical' in description:
        return "Lightning Activity"
    elif 'rain' in description or 'drizzle' in description or precip > 0.1:
...
def save_daily_backup(data_to_save, backup_dir="backups"):
    """
    Save the weather data to a JSON file named with today's date.
    ...
    filename = os.path.join(backup_dir, f"weather_backup_{today}.json")
